...
function drawObjInfo()
	...
	for i = 0,88 do
		...
		objX = memory.readbyte(0x1f0BC8+i)
		objY = memory.readbyte(0x1f0CD0+i)
		objVisibleX = memory.readbyte(0x1F0B70+i)*255
		objVisibleY = memory.readbyte(0x1F0C78+i)*255
		objDamage = memory.readbyte(0x1F1300+i)   --how much damage it will do if it collides with you
		objHealth = memory.readbyte(0x1F12A8+i-2) --something is wrong, -2 makes it right in some cases but makes others wrong
		objStatus = memory.readbyte(0x1f1250+i)   --collision status
...
function drawHitBoxes() 
	...
	for i = 0,88  do
		...
		adjustedX = objX - objVisibleX
		adjustedY = objY - objVisibleY
		objOffsetX = memory.readbyte(0x1F1930+i)
		objDirection = memory.readbyte(0x1F0AC0+i)
		objUnknownY = memory.readbyte(0x1f19E0+i)
		hitSizeX = memory.readbyte(0x1F1988+i)
		hitSizeY = memory.readbyte(0x1F1A38+i)
		
		if(objDirection == 1) then                     --9484
			objOffsetX = XOR(0xFF,objOffsetX)            --9488
			objOffsetX = objOffsetX+1                    --948A
		end

		off00 = 0 												             --942B
		if (AND(0x80, objOffsetX) == 128) then         --942F
			of00=AND(0xFF,(off00-1))                     --9431
		end
		
		hitBoxCornerX = AND(0xFF,(objX + objOffsetX))	 --9434
		hitBoxCornerY = AND(0xFF,(objY + objUnknownY)) --944F
		
		objStatus = memory.readbyte(0x1f1250+i)-- generally: 0 inert, 1 benign, 2 damageable/possibly damaging
		
		if(objStatus == 0) then
			agg.fillColor(0,0,255,96)
			agg.lineColor(0,0,0,192)
		elseif(objStatus == 1) then
			agg.fillColor(0,255,0,64)
			agg.lineColor(0,255,0,128)
		elseif(objStatus == 2) then
			agg.fillColor(255,0,0,80)
			agg.lineColor(255,0,0,128)
		end
...
function dispProperties(num)
	...
	agg.fillColor(255,200,200,100)

	ObjDamageInfo = {
		0x1f13b0,--5 means is being damaged
		0x1f1a90,--invulnerability countdown
		0x1f0f90,--if 0, animate.  seems to be used as a counter to temporarily freeze objs when damaged
		0x1f0b18,--flashing if equal to 0x10, white if equal to 0x86
		0x1f1ae8,
		0x1f1f18,
		0x1f11f8,
		0x1f12a8--health
	...
	for i = 1,8  do
		agg.text(10+(i*20),100, memory.readbyte(ObjDamageInfo[i]+num))
	end
	...
	for i = 1,4  do
		agg.text(10+(i*20),120, memory.readbyte(hitBoxInfo[i]+num))
	end
	
	ObjPositionInfo = {
		0x1F0B70, --offscreen or not
		0x1F0BC8, --x position
		0x1F0C20,
		0x1F0C78,
		0x1F0CD0, --y position
	...
	for i = 1,6  do
		agg.text(10+(i*20),140, memory.readbyte(ObjPositionInfo[i]+num))
	end
	
	collisionInfo = {
		0x1F0710, --compensated x
		0x1F0720, --compensated y
		0x1F0730  --compensated "visible"
	}
	
	for i = 1,3  do
		agg.text(10+(i*20),160, memory.readbyte(collisionInfo[i]+num))
	end
end
